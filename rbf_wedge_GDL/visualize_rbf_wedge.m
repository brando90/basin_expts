...
%% RBF original
tt_ = sum(t_.^2,2)';
degenerate_rbf_wedge = @(x) exp( -beta*eucledian(x,t_,tt_) ) * C_;
f_rbf_loss_surface_ = @(x) degenerate_rbf_wedge(x) + non_degenerate_rbf_pyramid(x);
